...
function syncPiloteQuetesFromLieu() {
  ...
  if (window.store) {
    ...
    // Quêtes créées à la main par le Pilote (« + Nouvelle quête ») : elles ne
    // viennent pas des solutions, on les recharge pour qu'elles survivent aux
    // reconstructions de la liste.
    store.where('quetes', r => r.custom === true && r.statut !== 'retiree').forEach(r => {
      if (PILOTE_QUETES_DEMO.some(q => q.id === r.id)) return;
      PILOTE_QUETES_DEMO.push({
